...
namespace PlanBuild
{
    [BepInPlugin("marcopogo.PlanBuild", "Plan Build", "0.0.5")]
    partial class PlanBuildMod : BaseUnityPlugin
    {
        public static ManualLogSource logger;
        public const string PlanBuildButton = "PlanBuild_PlanBuildMode";
        private const string ModName = "Plan Build";
        ...
        {
            string json = File.ReadAllText(GetAssetPath(languageConfig.Value));
            JsonObject localizationObject = JsonValue.Parse(json).AsJsonObject;
            foreach (KeyValuePair<string, JsonValue> pair in localizationObject)
            {
                LocalizationManager.Instance.RegisterTranslation(pair.Key, pair.Value);
            }
        }
        ...
        private void RegisterPieces(object sender, EventArgs e)
        {
            ...
            PieceManager.Instance.RegisterPieceTable(PlanHammerPrefabConfig.pieceTableName);
        ...
        {
            string text = Path.Combine(Paths.PluginPath, ModName, assetName);
            if (isDirectory)
            {
                if (!Directory.Exists(text))
                {
                    Assembly assembly = typeof(PlanBuildMod).Assembly;
